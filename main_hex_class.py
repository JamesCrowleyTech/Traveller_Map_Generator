...
class Hex:
    def __init__(self, true_width, true_height, mid_width, mid_height, subsector_width,
                 subsector_height, top, left, hex_edge_length, index, display, random=True, starport=None,
                 gas_giant=None, name=None, valid=None):
    ...
    def draw(self, x, y, colour=(255, 255, 255,), index_only=False):
        ...
        def display_name():
            ...
            whitespaces = font.render("".join([' ' for _ in range(self.hex_edge_length // 3)]), True, (255, 255, 255),
                                      (255, 255, 255))
        ...
        def display_starport():
            adj = 0 if self.is_random else 2
            font = pygame.font.SysFont('arial', int(self.hex_edge_length / 3.5))
            text = font.render(f' {self.starport} ', True, self.b, colour)
            text_rect = text.get_rect()
            text_rect.center = (x - adj, y - int(self.hex_edge_length / 3))
            self.display.blit(text, text_rect)

        def display_gas_giant():
            if self.gas_giant:
                pygame.draw.circle(self.display, (0, 0, 0), (x + self.hex_edge_length / 3 * 2, y - self.hex_edge_length / 5), self.hex_edge_length / 10)
            else:
                pygame.draw.circle(self.display, (255, 255, 255), (x + self.hex_edge_length / 3 * 2, y - self.hex_edge_length / 5), self.hex_edge_length / 10)
        ...
        if index_only and self.is_random:
            display_index()
            return
